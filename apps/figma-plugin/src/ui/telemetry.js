      ...
      function buildDiagnosticBundle(includeText = false) {
        const bundle = {
          ...
          state: {
            ...
            isSending: isSending || activeGenerations.size > 0,
      ...
      function showDiagnosticFeedback(message, failed = false) {
        if (!memberLoginOverlay.hidden) {
          memberLoginStatus.textContent = message;
          memberLoginStatus.classList.toggle("error", failed);
          return;
        }
        const original = diagnosticsButton.textContent;
        diagnosticsButton.textContent = message;
        window.setTimeout(() => {
      ...
      function copyDiagnosticInfo(issueId = "") {
        ...
        input.remove();
        if (copied) {
          showDiagnosticFeedback("已复制");
        } else if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(text)
            .then(() => showDiagnosticFeedback("已复制"))
            .catch(() => showDiagnosticFeedback("复制失败，请使用导出诊断包", true));
        } else {
          showDiagnosticFeedback("复制失败，请使用导出诊断包", true);
        }
