...
import type { AgentConfig, ColorPaletteConfig, MaterialPresetConfig, ModelConfig, OperationScenarioConfig, ScenarioAgentCaseConfig, ScenarioAgentConfig, ShapeArchitectureConfig } from "../../../packages/shared/src/index.ts";
import type { StoredConfig } from "./configStore.ts";
...
export function sortByLeadingNameNumber<T extends { name: string }>(items: T[]): T[] {
  return items
...
const seedConfigPath = path.join(seedDir, "defaultConfig.seed.json");
const seedMaterialsPath = path.join(seedDir, "defaultMaterials.seed.json");
const seedCatalogPath = path.join(seedDir, "defaultCatalog.seed.json");
...
const mockPreviewModel: ModelConfig = {
  id: "mock-preview", name: "Mock Preview", provider: "mock", model: "mock-preview",
  purpose: "image", quality: "auto", enabled: true,
  createdAt: "2026-05-08T16:27:18.677Z", updatedAt: "2026-05-28T08:00:00.000Z",
};

const gemini31ProModel: ModelConfig = {
  id: "gemini-3-1-pro", name: "Gemini 3.1 Pro", provider: "fintopia",
...
export function hydrateConfig(config: StoredConfig): StoredConfig {
  return {
