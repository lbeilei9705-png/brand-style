...
export function dedupeNegativePrompt(prompt: string): string {
...
export function cleanPositivePrompt(prompt: string): string {
...
export function shouldAnalyzeReferenceRoles(request: OptimizePromptRequest): boolean {
...
export function validateReferenceRolePlan(plan: ReferenceRolePlan | undefined, referenceAssets: InputAsset[]): ValidatedReferenceRolePlan | undefined {
  if (!plan) {
    return undefined;
  }
...
export function formatReferenceRoleRule(plan: ValidatedReferenceRolePlan | undefined): string {
  if (!plan) {
    return "";
  }
...
export function formatReferenceRoleNegativeRule(plan: ValidatedReferenceRolePlan | undefined): string {
...
export function buildReferenceRolePlanContent(request: OptimizePromptRequest): string {
...
function formatContext(context: PromptOrchestrationContext | undefined, options: { allowMaterialTransferColorShift?: boolean } = {}): string {
  ...
  const colorPalette = context.colorPalette
    ? context.colorPalette.prompt || context.colorPalette.description || context.colorPalette.name
    : "未选择配色";
  ...
  const shouldRemapManualPalette = Boolean(context.colorPalette)
    ...
    && !context.colorPalette?.description.includes("来自品牌预设");
  const colorInstruction = context.colorPalette
    ? isOriginalColorPalette
      ? "用户选择了原图色彩：最终提示词必须明确保持参考图原有色彩关系，不要使用品牌预设中的颜色描述进行改色；如果用户本轮输入中另有明确颜色或色值，以用户输入优先。"
      : shouldRemapManualPalette
      ? "已选择配色方案时，positive 只使用后台录入的配色提示词内容，不要额外补充色值、配色名称或解释性配色规则。"
...
export function buildUserContent(request: OptimizePromptRequest): string | Array<Record<string, unknown>> {
