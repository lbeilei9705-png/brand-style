  ...
  async optimize(request: OptimizePromptRequest, signal?: AbortSignal): Promise<PromptBundle> {
    ...
    try {
      response = await fetch(endpoint, {
        ...
        body: JSON.stringify(buildLanguagePayload(
          this.model,
          "你是一个多模态 3D 视觉生图 Prompt 编排器。你需要阅读用户选中的参考图，结合用户本轮输入、品牌预设、材质、形状和配色配置，生成最终可直接用于生图模型的提示词。不要引用历史对话上下文。如果有多张参考图，必须严格按“图1、图2、图3...”区分它们，用户提到某张图时不得混淆。你必须保持用户核心意图；优先级为：用户输入 > 自由搭配（形状/配色/材质）> 品牌预设 > 默认高清规则。颜色优先级为：用户输入的颜色/色值最高，当前启用配色第二，品牌预设中未作为默认配色启用的颜色描述最低；用户手动选择配色时，优先参考当前配色进行色彩转译，不要让配色规则压过参考图结构、图标数量、元素位置和色块关系；用户未手动选择配色时，可启用品牌预设默认配色。未选择任何配色方案时，按照原图色彩执行；清晰度只保留一条简短描述，不要在 positive 堆叠高清、4K、锐利、细节清晰等同义词，禁止项合并到 negative 且去重。只输出 JSON，字段为 positive 和 negative，不要输出 Markdown。",
